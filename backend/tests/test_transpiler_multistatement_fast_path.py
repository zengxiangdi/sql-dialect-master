...
def test_has_multiple_statements_skips_parser_without_semicolon(monkeypatch):
    def fail_if_parsed(*args, **kwargs):
        raise AssertionError("sqlglot.parse() should not run without a statement separator")

    monkeypatch.setattr("backend.core.transpiler.sqlglot.parse", fail_if_parsed)
