...
def test_parse_normalizes_dialect():
    response = client.post("/api/parse", json={"sql": "SELECT 1", "dialect": "MySQL"})
    assert response.status_code == 200
    assert response.json()["dialect"] == "mysql"


def test_parse_rejects_unknown_dialect():
    response = client.post("/api/parse", json={"sql": "SELECT 1", "dialect": "not_a_db"})
...
def test_nl2sql_normalizes_dialect():
    response = client.post(
        "/api/nl2sql", json={"text": "查询所有用户", "dialect": "MYSQL"}
    )
    assert response.status_code == 200
    assert response.json()["dialect"] == "mysql"
...
def test_nl2sql_rejects_unknown_dialect():
    response = client.post(
        "/api/nl2sql", json={"text": "查询所有用户", "dialect": "not_a_db"}
    )
    assert response.status_code == 400
    assert response.json()["success"] is False
