...
_cached_at = 0.0
_cached_probe_impl: Any = None


def _probe_allowed(request: Request) -> bool:
    configured = os.getenv(_PROBE_AUTH_ENV, "").strip()
    if configured:
        return hmac.compare_digest(request.headers.get(_PROBE_AUTH_HEADER, ""), configured)
...
def _failure_checks(code: str) -> dict[str, dict[str, str]]:
    return {name: {"status": "error", "code": code} for name in ("transpiler", "functions", "types", "nl2sql")}
...
async def _get_checks() -> dict[str, dict[str, Any]]:
    ...
    global _cached_checks, _cached_at, _cached_probe_impl
    probe_impl = _run_checks
    now = time.monotonic()
    if (
        _cached_checks is not None
        and _cached_probe_impl is probe_impl
    ...
    async with _PROBE_LOCK:
        ...
        if (
            ...
            return _cached_checks
        try:
            checks = await asyncio.wait_for(asyncio.to_thread(probe_impl), timeout=_PROBE_TIMEOUT_SECONDS)
        ...
        _cached_probe_impl = probe_impl
        return checks


def _result_response(checks: dict[str, dict[str, Any]], probe: str) -> JSONResponse:
    healthy = bool(checks) and all(check.get("status") == "ok" for check in checks.values())
    ...
    return JSONResponse(
        ...
        headers={"Cache-Control": "no-store"},
        content={"status": status, "version": _api_version(), "probe": probe, "checks": checks, "timestamp": datetime.now().isoformat()},
    )
