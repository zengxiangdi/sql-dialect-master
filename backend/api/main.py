...
app = FastAPI(
    ...
    openapi_tags=[
        ...
        {"name": "functions", "description": "📚 SQL function encyclopedia with 298 functions"},
        {"name": "types", "description": "🗂️ Data type mapping matrix (36 types × 12 databases)"},
...
@app.get("/", tags=["system"])
async def root():
    return {"name": API_TITLE, "version": API_VERSION, "status": "✅ Online", "description": "Enterprise-grade multi-database SQL conversion engine"}
...
async def parse_sql(request: ParseRequest):
    ...
    return {"success": True, "dialect": dialect, "elements": parser.extract_elements(request.sql), "timestamp": datetime.now().isoformat()}
...
async def map_type(request: TypeMapRequest):
    source_dialect = normalize_dialect(request.source_dialect, "source dialect")
    target_dialect = normalize_dialect(request.target_dialect, "target dialect")
    return {"success": True, **type_mapper.suggest_type(request.type_name, source_dialect, target_dialect), "timestamp": datetime.now().isoformat()}
...
async def health_check():
    services = {"transpiler": {"status": "✅ healthy", "rules": 40, "description": "SQL conversion engine"}, "functions": {"status": "✅ healthy", "count": len(func_encyclopedia.functions), "description": "Function encyclopedia"}, "types": {"status": "✅ healthy", "count": len(type_mapper.mappings), "description": "Type mapping service"}, "nl2sql": {"status": "✅ healthy", "description": "Natural language processor"}}
    all_healthy = all("healthy" in s["status"] for s in services.values())
    return {"status": "✅ healthy" if all_healthy else "⚠️ degraded", "version": API_VERSION, "uptime": "Available", "timestamp": datetime.now().isoformat(), "services": services}
...
async def sdm_exception_handler(request: Request, exc: SDMException):
    status_code = 400 if isinstance(exc, UnsupportedDialectError) else 400
